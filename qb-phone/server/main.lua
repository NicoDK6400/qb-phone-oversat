...
RegisterNetEvent('qb-phone:server:TransferMoney', function(iban, amount)
    ...
    if result[1] ~= nil then
        ...
        TriggerClientEvent('QBCore:Notify', src, "Denne konto eksistere ikke!", "error")
    end
end)
...
RegisterNetEvent('qb-phone:server:sendPing', function(data)
    local src = source
    if src == data then
        TriggerClientEvent("QBCore:Notify", src, "Du kan ikke ping dig selv", "error")
    end
end)

-- Command

QBCore.Commands.Add("setmetadata", "Sæt spiller Metadata (God Only)", {}, false, function(source, args)
    local Player = QBCore.Functions.GetPlayer(source)
    if args[1] then
...
QBCore.Commands.Add('bill', 'Giv bøde', {{name = 'id', help = 'Spiller ID'}, {name = 'amount', help = 'Bøde størrelse'}}, false, function(source, args)
        ...
        if billed ~= nil then
            if biller.PlayerData.citizenid ~= billed.PlayerData.citizenid then
                if amount and amount > 0 then
                    ...
                    TriggerClientEvent('QBCore:Notify', source, 'Skal være en værdi over 0', 'error')
                end
            else
                TriggerClientEvent('QBCore:Notify', source, 'Du kan ikke sende en regning til dig selv', 'error')
            end
        else
