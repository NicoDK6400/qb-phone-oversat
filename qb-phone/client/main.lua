...
local function OpenPhone()
    QBCore.Functions.TriggerCallback('qb-phone:server:HasPhone', function(HasPhone)
        if HasPhone then
            ...
            QBCore.Functions.Notify("Du har ingen mobil", "error")
        end
    end)
end
...
RegisterNetEvent('qb-phone:client:AddNewSuggestion', function(SuggestionData)
    ...
    SendNUIMessage({
        ...
        PhoneNotify = {
            title = "Phone",
            text = "Du har en foreslået kontakt!",
            icon = "fa fa-phone-alt",
            color = "#04b543",
            timeout = 1500,
        },
...
RegisterNetEvent('qb-phone:client:GetMentioned', function(TweetMessage, AppAlerts)
    ...
    SendNUIMessage({ action = "PhoneNotification", PhoneNotify = { title = "Du er blevet nævnt i en tweet!", text = TweetMessage.message, icon = "fab fa-twitter", color = "#1DA1F2", }, })
