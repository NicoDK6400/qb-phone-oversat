...
$(document).on('click', '.racing-race', function(e){
    ...
    if (!RaceData.RaceData.Started || IsRacer) {
        ...
        QB.Phone.Notifications.Add("fas fa-flag-checkered", "Racing", "Race er startet..", "#1DA1F2");
    }
});
...
function SetupRaces(Races) {
    ...
    if (Races.length > 0) {
        ...
        $.each(Races, function(i, race){
            var Locked = '<i class="fas fa-unlock"></i> Ikke startet';
            if (race.RaceData.Started) {
                Locked = '<i class="fas fa-lock"></i> Startet';
            }
            var LapLabel = "";
            if (race.Laps == 0) {
                LapLabel = "SPRINT"
            ...
            if (InRace) {
                if (!Creator) {
                    Buttons = '<div class="race-buttons"> <div class="race-button" id="quit-race" data-toggle="racetooltip" data-placement="right" title="Afslut"><i class="fas fa-sign-out-alt"></i></div>';
                } else {
                    if (!race.RaceData.Started) {
    ...
    $.post('https://qb-phone/IsAuthorizedToCreateRaces', JSON.stringify({}), function(data){
        if (data.IsAuthorized) {
            if (!data.IsBusy) {
                ...
                QB.Phone.Notifications.Add("fas fa-flag-checkered", "Racing", "Du er igang med at opsætte et race..", "#1DA1F2");
            }
        } else {
            QB.Phone.Notifications.Add("fas fa-flag-checkered", "Racing", "Du har ikke rettigheder til at oprette et race..", "#1DA1F2");
        }
    });
});
...
$(document).on('click', '#racing-create-accept', function(e){
    ...
    if (TrackName !== "" && TrackName !== undefined && TrackName !== null) {
        ...
        $.post('https://qb-phone/IsAuthorizedToCreateRaces', JSON.stringify({
            ...
            if (data.IsAuthorized) {
                if (data.IsNameAvailable) {
                    ...
                    QB.Phone.Notifications.Add("fas fa-flag-checkered", "Racing", "Navnet er ikke tilgængeligt..", "#1DA1F2");
                }
            } else {
                QB.Phone.Notifications.Add("fas fa-flag-checkered", "Racing", "Du har ikke rettigheder til at oprette et race..", "#1DA1F2");
            }
        ...
        QB.Phone.Notifications.Add("fas fa-flag-checkered", "Racing", "Du skal indtaste et navn til race..", "#1DA1F2");
    }
});
                            ...
                            $.post('https://qb-phone/CanRaceSetup', JSON.stringify({}), function(CanSetup){
                                if (CanSetup) {
                                    ...
                                    $(".racing-setup").animate({
                                        ...
                                        $(".racing-setup-information-distance").html('Vælg route');
                                        $(".racing-setup-information-creator").html('Vælg route');
                                        $(".racing-setup-information-wr").html('Vælg route');
                                        $(".racing-setup-laps").val("");
                                        $('.dropdown').find('input').removeAttr('value');
                                        $('.dropdown').find('span').text("Vælg route");
                                    });
                                } else {
                                    QB.Phone.Notifications.Add("fas fa-flag-checkered", "Racing", "Der kan ikke være nogle ..", "#1DA1F2");
                                }
                            });
                        } else {
                            QB.Phone.Notifications.Add("fas fa-flag-checkered", "Racing", "Indtast antal runder..", "#1DA1F2");
                        }
                    } else {
                        QB.Phone.Notifications.Add("fas fa-flag-checkered", "Racing", "Du har ikke valgt en route..", "#1DA1F2");
                    }
                }
            })
        } else {
            QB.Phone.Notifications.Add("fas fa-flag-checkered", "Racing", "Du har et aktivt race..", "#1DA1F2");
        }
    });
});
